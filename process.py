...
def main():
    ...
    # Run checks
    df, views = check_and_adapt_csv(csv_path)
    ...
    for index, row in df.iterrows:
        ...
        for view in views:
            ...
            img_cropped_clahe = cv2.normalize(img_cropped_clahe,None,0,255,cv2.NORM_MINMAX).atype(np.uint8)
            # Save images
            maybe_make_dir(os.path.join(paths['cropped'],view))
            maybe_make_dir(os.path.join(paths['cropped_clahe'],view))
            if(file_format_orig in ['jpg','png']):
                imsave(os.path.join(paths['cropped'],view,row[f"img_path_{view}"]),img_cropped)
                imsave(os.path.join(paths['cropped_clahe'],view,row[f"img_path_{view}"]),img_cropped_clahe)
            if(file_format_orig in ['nifti']):
                convert_2d_image_array_to_nifti(img_cropped, os.path.join(paths['cropped'],view,row[f"img_path_{view}"][:-5]))
                convert_2d_image_array_to_nifti(img_cropped_clahe, os.path.join(paths['cropped_clahe'],view,row[f"img_path_{view}"][:-5]))   
